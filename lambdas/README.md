...
- To run the build process, run `bun run build`
- An individual Lambda can be built by running `bun run build -- --lambda {lambda-name}`

### Packaging Process

- Each Lambda is packaged as a ZIP archive
- The archives can be found in the `dist/` directory
- To create the ZIP archives, run `bun run package`
- An individual Lambda can be packaged by running `bun run package -- --lambda {lambda-name}`

### Local Deployment

- Lambdas are deployed via Terraform using `archive_file` data source
- To deploy all Lambdas, run `bun run local:terraform:apply`
...
- Deploy via Terraform: `bun run local:terraform:apply`
