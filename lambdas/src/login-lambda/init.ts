...
const envVars: LoginEnvVariables = {
  healthCheckMinimumAge: Number.parseInt(
    retrieveOptionalEnvVariable('HEALTH_CHECK_MINIMUM_AGE', '40')
  ),
  healthCheckMaximumAge: Number.parseInt(
    retrieveOptionalEnvVariable('HEALTH_CHECK_MAXIMUM_AGE', '74')
  ),
...
interface LoginEnvVariables {
  healthCheckMinimumAge: number;
  healthCheckMaximumAge: number;
...
export async function init(): Promise<LoginLambdaDependencies> {
  ...
  const secretManagerClient = new AwsSecretsClient('eu-west-1');
