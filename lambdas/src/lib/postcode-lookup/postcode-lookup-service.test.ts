...
describe('PostcodeLookupService', () => {
  ...
  describe('performLookup', () => {
    ...
    it('should throw error when client lookup fails', async () => {
      ...
      await expect(service.performLookup('SW1A 1AA')).rejects.toThrow('Failed to lookup postcode: API error');
    });

    it('should handle non-Error exceptions from client', async () => {
      mockClient.lookupPostcode.mockRejectedValue('String error');

      await expect(service.performLookup('SW1A 1AA')).rejects.toThrow('Failed to lookup postcode: Unknown error');
    });
    ...
    it('should reject postcode that is too long', async () => {
      await expect(service.performLookup('SW1A 1AA 1BB')).rejects.toThrow('Invalid postcode format');
    });
  });
});
