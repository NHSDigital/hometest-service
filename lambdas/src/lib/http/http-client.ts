...
export class HttpClient extends Service {
  ...
  constructor(commons: Commons, options?: HttpClientOptions) {
  ...
  async doPostRequestWithStatus<TBody, TResponse>(
    ...
    try {
      this.logger.debug('about to trigger POST request to endpoint', {
        endpointUrl
      });
      ...
      this.logger.error('doPostRequest error', errorDetails);
  ...
  async doPutRequestWithStatus<TBody, TResponse>(
    ...
    try {
      this.logger.debug('about to trigger PUT request to endpoint', {
        endpointUrl
      });
      const timeoutInSeconds = this.options?.timeoutInSeconds ?? 30;
      ...
      const errorDetails = this.createErrorDetailsObject(error);
      this.logger.error('doPutRequestWithStatus error', errorDetails);
  ...
  async deleteRequest<TResponse>(
    ...
    try {
      this.logger.debug('about to trigger DELETE request to endpoint', {
        endpointUrl: this.removeGetParams(endpointUrl)
      });
      ...
      const errorDetails = this.createErrorDetailsObject(error);
      this.logger.error('deleteRequest error', errorDetails);
  ...
  async doGetRequestWithHeaders<TResponse>(
    ...
    this.logger.debug('about to trigger GET request to endpoint', {
      endpointUrl: this.removeGetParams(endpointUrl)
    });
  ...
  async getRequest<TResponse>(
    ...
    try {
      ...
      this.logger.error('getRequest error', {
        errorDetails: {
          ...errDetails,
          name: errJson.name,
          code: errJson.code,
