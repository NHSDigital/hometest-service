...
printf 'NEXT_PUBLIC_BACKEND_URL=%s\nNEXT_PUBLIC_NHS_LOGIN_AUTHORIZE_URL=%s\nNEXT_PUBLIC_USE_WIREMOCK_AUTH=true\n' "$LOCALHOST_BACKEND_BASE_URL" "$NHS_LOGIN_AUTHORIZE_URL" > ./ui/.env.local
...
if grep -q '^API_BASE_URL=' "$TESTS_ENV_FILE"; then
	sed -E "s|^API_BASE_URL=.*$|API_BASE_URL=$LOCALHOST_BACKEND_BASE_URL|" "$TESTS_ENV_FILE" > "${TESTS_ENV_FILE}.tmp"
	mv "${TESTS_ENV_FILE}.tmp" "$TESTS_ENV_FILE"
else
	printf '\nAPI_BASE_URL=%s\n' "$LOCALHOST_BACKEND_BASE_URL" >> "$TESTS_ENV_FILE"
fi
