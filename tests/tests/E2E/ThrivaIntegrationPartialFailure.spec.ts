...
test(
  'E2E - Thriva Integration Partial Failure',
  ...
  async ({
    ...
    await test.step('Place a blood test order', async () => {
    ...
    if (!config.integratedEnvironment) {
      await test.step('Simulate Thriva response to results API', async () => {
        ...
        const labResult = getLabResults(LabResultsData.PartialFailureHbA1c);
      ...
      await test.step(`Lab results got processed and health check status got updated to GP_UPDATE_SUCCESS`, async () => {
        const healthCheck =
          await dbHealthCheckService.getLatestHealthCheckItemsByNhsNumber(
            testedUser.nhsNumber
          );
