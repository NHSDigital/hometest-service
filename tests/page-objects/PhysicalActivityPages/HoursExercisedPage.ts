...
export class HoursExercisedPage extends HTCPage {
  ...
  constructor(page: Page) {
    ...
    this.pageHeader = page.locator(
      'h1:has-text("How many hours do you exercise in a typical week?")'
    );
    this.backLink = page.getByRole('link', { name: 'Back' });
    const errorText = 'Select how many hours you exercised last week';
  ...
  async selectExerciseHoursOptions(selection: ExerciseOptions): Promise<void> {
    switch (selection) {
      case ExerciseOptions.NONE:
        await this.noneRadio.check();
        break;

      case ExerciseOptions.LESS_THAN_ONE_HOUR:
        await this.LessThanOneRadio.check();
        break;
      case ExerciseOptions.MORE_THAN_ONE_LESS_THAN_THREE:
        await this.MoreThanOneLessThanThreeRadio.check();
        break;
      case ExerciseOptions.THREE_OR_MORE:
  ...
  async selectExerciseOptionsAndClickContinue(
    selection: ExerciseOptions
  ): Promise<void> {
    await this.selectExerciseHoursOptions(selection);
    await this.clickContinueButton();
  }

  getExpectedTitleHeading(): string {
    return pageTitlesMap[JourneyStepNames.HoursExercisedPage];
  }
}
