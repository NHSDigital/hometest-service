...
export class HoursHouseworkPage extends HTCPage {
  ...
  constructor(page: Page) {
    ...
    this.pageHeader = page.locator(
      'h1:has-text("How many hours do you spend on housework or childcare in a typical week? (optional)")'
    );
    this.backLink = page.getByRole('link', { name: 'Back' });
    const errorText =
      'Select how many hours you spent on housework or childcare last week';
  ...
  async selectHouseworkHoursOptions(
    selection: HouseworkOptions
  ): Promise<void> {
    switch (selection) {
      case HouseworkOptions.NONE:
        await this.noneRadio.check();
        break;

      case HouseworkOptions.LESS_THAN_ONE_HOUR:
        await this.LessThanOneRadio.check();
        break;
      case HouseworkOptions.MORE_THAN_ONE_LESS_THAN_THREE:
        await this.MoreThanOneLessThanThreeRadio.check();
        break;
      case HouseworkOptions.THREE_OR_MORE:
  ...
  async selectHouseworkOptionsAndClickContinue(
    selection?: HouseworkOptions
  ): Promise<void> {
    if (selection !== null && selection !== undefined) {
      await this.selectHouseworkHoursOptions(selection);
    }
    await this.clickContinueButton();
  }

  getExpectedTitleHeading(): string {
    return pageTitlesMap[JourneyStepNames.HoursHouseworkPage];
  }
}
